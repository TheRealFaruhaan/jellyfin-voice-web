...
/**
 * Message types emitted by the custom server build (voice chat, media acquisition).
 * They intentionally live outside the SDK's OutboundWebSocketMessageType enum.
 */
const CustomMessageType = {
    VoiceChatSignal: 'VoiceChatSignal',
...
function subscribeToApiClient(apiClient) {
    const subscriptions = [
        ...
        // Custom server message types. These are not in the SDK's
        // OutboundWebSocketMessageType enum, but the socket dispatches on the raw
        // MessageType string, so subscribing by literal works.
        apiClient.subscribe([CustomMessageType.VoiceChatSignal], ({ Data }) => {
            pluginManager.firstOfType(PluginType.SyncPlay)?.instance.Manager.voiceChatCore?.handleSignal(Data);
        }),
        apiClient.subscribe([CustomMessageType.VoiceChatState], ({ Data }) => {
            Events.trigger(serverNotifications, CustomMessageType.VoiceChatState, [apiClient, Data]);
        }),
        apiClient.subscribe([CustomMessageType.TorrentProgress], ({ Data }) => {
            Events.trigger(serverNotifications, CustomMessageType.TorrentProgress, [apiClient, Data]);
        })
    ];
